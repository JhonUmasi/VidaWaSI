...
async function inicializarDetalle() {
    try {
        ...
        if (productoElegido) {
            ...
            document.getElementById('vista-detalle').innerHTML = "<h2>Prenda no encontrada</h2>";
        }
    } catch (error) {
...
function dibujarDetalle(prenda) {
    ...
    contenedor.innerHTML = `
        <div class="grid-detalle">
            <div class="detalle-imagen">
                <img src="${prenda.foto}" alt="${prenda.nombre}" onerror="this.src='https://via.placeholder.com/600x800?text=Pendiente'">
            </div>
    ...
    // Lógica para actualizar el enlace de WhatsApp dinámicamente
    ...
    // Actualizar enlace de inicio y cada vez que cambien los selects
    actualizarEnlaceWa();
...
function mostrarRecomendaciones(categoriaActual, idActual) {
    ...
    // Filtrar: misma categoría, que no sea el producto actual, y que esté disponible
